...
my_config_values = read_parameters_from_file(TRANSACTION_PARAMS_PATH + '/promospots-ads/transaction/config_historical.yaml',ENVIRONMENT)
...
DAG_NAME="ads_gcs_to_bq_avro_data_load_hist_adhoc"
...
dag = DAG(
    dag_id=DAG_NAME,
    description='ADS AdMedia Reporting-Transaction Historical Load job',
    catchup=False,
    default_args=default_args,
    schedule_interval=SCHEDULE_INTERVAL,    # schedule_interval=None,  "@once"
    max_active_runs= 1,
    template_searchpath=TRANSACTION_PARAMS_PATH + '/promospots-ads/transaction')

def get_execution_date(execution_dt):
    if my_config_values['transaction_dt_plus_one_day'] == 'None':
        transaction_dt_plus_one = execution_dt.date()
    else:
        LOAD_DATE = datetime.strptime(my_config_values['transaction_dt_plus_one_day'],'%Y-%m-%d')
        transaction_dt_plus_one = LOAD_DATE.date()   
    return transaction_dt_plus_one
...
#Storage client within this project/composer scope
...
def raise_skipping_exception_email(environment, email_type, missing_avro_files, dag_name, product="promospots-ads", assets_tag="ADS:TRANSACTION-LOAD"):
    ...
    email_body += """<strong>Warning</strong>: No data exists to process, so this job is skipping all the tasks.</br></br>"""
...
def missing_avro_files_failure_email(environment, email_type, missing_avro_files, dag_name, product="promospots-ads", assets_tag="ADS:TRANSACTION-LOAD"):
    email_subject = """%s %s for %s %s %s""" %(environment, email_type, product, assets_tag, dag_name)
    email_body = """<p style='color:#FF0000'>*** This email has been generated automatically - DO NOT reply directly ***</p>"""
...
def duplicate_hours_files_email(environment, email_type, duplicate_hour_files, dag_name, product="promospots-ads", assets_tag="ADS:TRANSACTION-LOAD"):
    email_subject = """%s %s for %s %s %s""" %(environment, email_type, product, assets_tag, dag_name)
    email_body = """<p style='color:#FF0000'>*** This email has been generated automatically - DO NOT reply directly ***</p>"""
...
#Step0: Moving Archive files to Ingress bucket
def find_manifest_files_from_archive(**kwargs):
    ...
    kv_feed = []
    standard_feed = []
    print("DAG_START_DATE:",kwargs['dag_run'].start_date)
    today_dt = get_execution_date(kwargs['dag_run'].start_date)
    yesterday_dt = today_dt - timedelta(1)
    
    kv_feed_regex = prefix + "auction_kv_labels_feed-11303-" + yesterday_dt.strftime("%Y%m%d") + "\w+"
    standard_feed_regex = prefix + "standard_feed-11303-" + yesterday_dt.strftime("%Y%m%d") + "\w+"
    ...
    if not (kv_feed and standard_feed):
        raise_skipping_exception_email(environment=ENVIRONMENT, email_type="Warning", missing_avro_files=None, dag_name=DAG_NAME)
...
def fetch_manifest_files_from_archive(**kwargs):
    ...
    move_file(ARCHIVE_BUCKET_NAME, INGRESS_BUCKET_NAME, source_files=kv_feeds, destination_files=kv_feeds)
    print("kv_feed_length:",len(kv_feeds),"standard_feed_length:",len(standard_feeds))

    # move standard feeds from archival to ingress bucket
    move_file(ARCHIVE_BUCKET_NAME, INGRESS_BUCKET_NAME, source_files=standard_feeds, destination_files=standard_feeds)
    return True
...
def fetch_kv_feeds_from_archival(**kwargs):
    today_dt = get_execution_date(kwargs['dag_run'].start_date)
    yesterday_dt = today_dt - timedelta(1)
    kv_feed_common_path = "feeds/auction_kv_labels_feed/{date}/".format(date=yesterday_dt.strftime("%Y%m%d"))
...
def scan_bucket(**kwargs):
    ...
    today_dt = get_execution_date(kwargs['dag_run'].start_date)
    yesterday_dt = today_dt - timedelta(1)
    kv_feed_regex = prefix + KV_FEED_PREFIX + yesterday_dt.strftime("%Y%m%d") + "\w+"
    standard_feed_regex = prefix + STD_FEED_PREFIX + yesterday_dt.strftime("%Y%m%d") + "\w+"
    kv_feed = []
    standard_feed = []
    for blob in blobs:
        if re.match(kv_feed_regex, blob.name):
...
#Step2: Find deduplicate manisfest files based on hour for the same day
def filter_duplicate_hours(feed_path:list):
    filter_hours_dict = {}
    duplicate_file_list = []
...
#Step2.1: deduplicate manisfest files for both kv and standard feeds
def remove_duplicate_hours(**kwargs):
    ...
    if duplicate_file_list:
        duplicate_hours_files_email(environment=ENVIRONMENT, email_type="Warning", duplicate_hour_files=duplicate_file_list, dag_name=DAG_NAME)

    return {"kv_feed": filtered_kv_feeds, "standard_feed": filtered_standard_feeds}

#Step3: Supporting function for moving files from bucket to bucket  
...
#Step4: Supporting function for manifest files insert query
def prepared_data_manifest_stg(feed_type:list):
    ...
    for file_path in valid_files:
        ...
        row = f"('{manifest_name}', '{transaction_dt}', {transaction_hr}, '{duplicate_file_check}', '{load_ts}')"
        values.append(row)
    ...
    for file_path in duplicate_files:
        ...
        transaction_dt = datetime.strptime(file_path.split("-")[2], "%Y%m%d%H").date()
        transaction_hr = datetime.strptime(file_path.split("-")[2], "%Y%m%d%H").hour
        duplicate_file_check = "Y"
        row = f"('{manifest_name}', '{transaction_dt}', {transaction_hr}, '{duplicate_file_check}', '{load_ts}')"
        values.append(row)
    return values

#Step4.1: Load manifest file names into manifest staging table
def load_manifest_table(**kwargs):
    ...
    insert_kv_feeds_sql = f"INSERT INTO `{PIPELINE_PROJECT_ID}.{PIPELINE_DATASET_ID}.manifest_log_stg` VALUES " + ",".join(
    ...
    insert_standard_feeds_sql = f"INSERT INTO `{PIPELINE_PROJECT_ID}.{PIPELINE_DATASET_ID}.manifest_log_stg` VALUES " + ",".join(
        insert_job_kv_feeds_values)
    print(insert_kv_feeds_sql)
    ...
    insert_job = insert_client.query(insert_kv_feeds_sql,
                                     location="US", job_config=job_config)
    insert_job.result()
    insert_job = insert_client.query(insert_standard_feeds_sql,
                                     location="US", job_config=job_config)
    insert_job.result()
...
def check_valid_avro_files(**kwargs):
    ...
    if missing_avro_files:
        missing_avro_files_failure_email(environment=ENVIRONMENT, email_type="Warning", missing_avro_files=missing_avro_files, dag_name=DAG_NAME)
...
def missing_placement_warning_email(environment, email_type, result_rows, dag_name, product="promospots-ads", assets_tag="ADS:TRANSACTION-LOAD"):
    ...
    email_body += """<ul>"""
    email_body += """<li>For each standard feed tag_id above, add missing row of placement information to placements table in Dev; update config_historical.yaml file transaction_dt_plus_one_day variable to the date the job was supposed to run; promote changes to Cert & Prod.</br>"""
    email_body += """<li>Have DnA SREs manually run adhoc DAG for historical transaction_dt.</br>"""
    email_body += """</ul>"""
...
def archive_valid_avro_files(**kwargs):
    ...
    archive_avro_files(avro_dt_directory_path='feeds/auction_kv_labels_feed/' + yesterday_dt_str)
    archive_avro_files(avro_dt_directory_path='feeds/standard_feed/' + yesterday_dt_str)
...
def history_files_warning_email(environment, email_type, history_files_list, dag_name, product="promospots-ads", assets_tag="ADS:TRANSACTION-LOAD"):
    ...
    email_body += """<strong>Action</strong>: Please validate historical file(s) and, if needed, request Operations to run adhoc DAG for historical file(s)."""

    send_warning_email(recipient_email=WARNING_RECIPIENT_EMAIL, subject=email_subject, body=email_body)

#Step12: History file check
def check_history_files(**kwargs):
    prefix = "manifests/"
    blobs = storage_client_impersonated.list_blobs(INGRESS_BUCKET_NAME, prefix=prefix, delimiter="/")
    today_dt = get_execution_date(kwargs['dag_run'].start_date)
    ...
    for blob in blobs:        
        ...
        if len(file_split) == 4:
            file_dt = datetime.strptime(file_split[2], "%Y%m%d%H").date()
            file_ts = datetime.strptime(file_split[3].split(".")[0], "%Y%m%d%H%M%S").date()
            print("FILE_TS:", file_ts, "FILE_DT:", file_dt)
            if file_dt < yesterdays_dt:
                print("found history file:{}".format(blob.name))
                history_files_list.append(blob.name)
    print(history_files_list)
...
def api_post(**kwargs):
    ...
    apn_start_date = get_execution_date(kwargs['dag_run'].start_date) - timedelta(1)
    apn_end_date = get_execution_date(kwargs['dag_run'].start_date)
...
def download_file(**kwargs):
    ...
    if report_id is not None:
        ...
        if len(data.strip()) > 2:
            print(data)
            return data
    transaction_dt = get_execution_date(kwargs['dag_run'].start_date) - timedelta(1)
...
def transaction_report_email(**kwargs):
    today_dt = get_execution_date(kwargs['dag_run'].start_date)
    transaction_dt = today_dt - timedelta(1)
...
fetch_manifest_files_from_archive = PythonOperator(
...
fetch_kv_feeds_from_archival = PythonOperator(
...
fetch_standard_feeds_from_archival = PythonOperator(
...
find_manifest_files_from_archive = PythonOperator(
...
archive_hist_move_complete = dummy_operator.DummyOperator(
    task_id='archive_hist_move_complete',
...
load_manifest_file_to_table = PythonOperator(
...
check_history_files = PythonOperator(
    task_id='check_history_files',
    python_callable=check_history_files,
    provide_context=True,
    labels=GCP_COST_LABELS_PYTHON_API,
    dag=dag
)
...
#Fetch and move archive files to ingress folder
start >> find_manifest_files_from_archive >> [fetch_manifest_files_from_archive, fetch_kv_feeds_from_archival, fetch_standard_feeds_from_archival] >> archive_hist_move_complete

#Avro files process tasks
archive_hist_move_complete >> scan_manifest_files >> remove_duplicate_hour_files >> prepare_avro_files >> [archive_duplicate_files, check_valid_avro_files]
check_valid_avro_files >> [load_manifest_file_to_table, load_std_avro_files_to_bq, load_kv_avro_files_to_bq] >> run_query_placement_tag >> avro_load_complete
...
transaction_report_email >> [archive_valid_manifest_files, archive_valid_avro_files] >> check_history_files >> end