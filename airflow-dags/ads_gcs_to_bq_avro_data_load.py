...
my_config_values = read_parameters_from_file(TRANSACTION_PARAMS_PATH + '/promospots-ads/transaction/config.yaml',ENVIRONMENT)
...
DAG_NAME="ads_gcs_to_bq_avro_data_load"
...
def missing_manifest_failure_email_job_does_not_fail(environment, email_type, missing_manifest_files_count, transaction_dt, dag_name,product="promospots-ads", assets_tag="ADS:TRANSACTION-LOAD"):
    ...
    email_body += """<ul>
                        <li> Run this SQL to determine which manifest files are missing:
                            <ul>
                                <li>SELECT * FROM `%s.%s.manifest_log_stg` WHERE transaction_dt IN ( 
    SELECT MAX(transaction_dt) FROM `%s.%s.manifest_log_stg` WHERE transaction_dt != ( 
    SELECT MAX(transaction_dt) FROM `%s.%s.manifest_log_stg`)) 
...
def load_manifest_staging_table():
    ...
    insert_sql_query = "INSERT INTO `{PIPELINE_PROJECT_ID}.{PIPELINE_DATASET_ID}.manifest_log_stg` VALUES ".format(PIPELINE_PROJECT_ID=PIPELINE_PROJECT_ID,PIPELINE_DATASET_ID=PIPELINE_DATASET_ID)
...
def notify_history_files_list(bq_client, transaction_dt):
    query = "SELECT manifest_name FROM `{PIPELINE_PROJECT_ID}.{PIPELINE_DATASET_ID}.manifest_log_stg` " \
            "WHERE transaction_dt='{transaction_dt}'".format(PIPELINE_PROJECT_ID=PIPELINE_PROJECT_ID,PIPELINE_DATASET_ID=PIPELINE_DATASET_ID,transaction_dt=transaction_dt)
...
def load_control_table():
    ...
    FROM `{PIPELINE_PROJECT_ID}.{PIPELINE_DATASET_ID}.manifest_log_stg` \
...
def update_processed_flag_for_current_transaction():
    ...
    manifest_statement = (
        "UPDATE `{PIPELINE_PROJECT_ID}.{PIPELINE_DATASET_ID}.manifest_log_stg` \
        SET processed_flag = '{processed_flag}', load_ts = '{load_ts}' \
        WHERE transaction_dt = '{transaction_dt}'".format(PIPELINE_PROJECT_ID=PIPELINE_PROJECT_ID,PIPELINE_DATASET_ID=PIPELINE_DATASET_ID,transaction_dt=transaction_dt,processed_flag=processed_flag,load_ts=datetime.strftime(load_ts,'%Y-%m-%d %H:%M:%S')))
...
destination_dataset_table="{PIPELINE_PROJECT_ID}.{PIPELINE_DATASET_ID}.manifest_log_stg".format(PIPELINE_PROJECT_ID=PIPELINE_PROJECT_ID,PIPELINE_DATASET_ID=PIPELINE_DATASET_ID),
sql="SELECT * FROM `{PIPELINE_PROJECT_ID}.{PIPELINE_DATASET_ID}.manifest_log_stg` where 1=0".format(PIPELINE_PROJECT_ID=PIPELINE_PROJECT_ID,PIPELINE_DATASET_ID=PIPELINE_DATASET_ID),
bigquery_conn_id='google_cloud_default',
impersonation_chain=SERVICE_ACCOUNT,
use_legacy_sql=False,
