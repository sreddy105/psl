...
DAG_NAME="ads_aafs_customer_file_to_bq_data_load_test"
target_dag_id = "ads_aafs_travel_agency_bq_to_bq_data_refresh_test"
...
my_config_values = read_parameters_from_file(AAFS_PARAMS_PATH + '/ads-aafs-test/aafs/config_aafs_test.yaml',ENVIRONMENT)
...
SECONDS_TO_WAIT_BETWEEN_DAG_STATE_CHECKS = my_config_values['seconds_to_wait_between_dag_state_checks']
TOTAL_SECONDS_TO_CHECK_DAG_STATE = my_config_values['total_seconds_to_check_dag_state']
...
dag = DAG(
    ...
    schedule_interval=None,  # SCHEDULE_INTERVAL, None, "30 15 * * *" (in UTC time) = 10:30 AM CST daily, may use monthly schedule in the future  "*/31 15 5 * *" (in UTC time) = 10:31 AM CST on 5th of each month 
    max_active_runs= 1,
    template_searchpath=AAFS_PARAMS_PATH + '/ads-aafs-test/aafs')
...
def send_failure_email_when_AAFS_refresh_running():
    thours, tmins, tsecs = TOTAL_SECONDS_TO_CHECK_DAG_STATE.split("*")
    timeout_seconds = int(thours)*int(tmins)*int(tsecs) # hours * mins * seconds
    hours = round(timeout_seconds/3600,2)
    
    email_type = 'Failure'
    email_subject = """%s %s for ads-aafs ADS:ADS:AAFS-LOAD-GCP %s""" %(ENVIRONMENT, email_type, DAG_NAME)
    
    email_body = """<p style='color:#FF0000'>*** This email has been generated automatically - DO NOT reply directly ***</p>"""
    email_body += """<strong>Failure</strong>:</br>"""
    email_body += """AAFS Load DAG cannot run until AAFS Refresh DAG has completed. This is because the two jobs both populate three of the same target tables and six of the same staging tables.</br>"""
    email_body += """</br><strong>Actions</strong>:</br>"""
    email_body += """1.	Check why AAFS Refresh DAG (%s) has been running for at least %s hours.  </br>"""%(target_dag_id,hours)
    email_body += """2. Once AAFS Refresh DAG is stopped (either successfully or in a failed state), <strong>manually trigger</strong> AAFS Load DAG to start <strong>from the beginning</strong>."""
    
    send_warning_email(recipient_email=FAILURE_RECIPIENT_EMAIL, subject=email_subject, body=email_body)

def dag_run_status_check():
    whours, wmins, wsecs = SECONDS_TO_WAIT_BETWEEN_DAG_STATE_CHECKS.split("*")
    waiting_time = int(whours)*int(wmins)*int(wsecs) # hours * mins * seconds
    thours, tmins, tsecs = TOTAL_SECONDS_TO_CHECK_DAG_STATE.split("*")
    timeout = int(thours)*int(tmins)*int(tsecs) # hours * mins * seconds
    ...
    while dag_runs[-1].get_state() == 'running' and counter>0: 
        ...
        counter=counter-1
        if counter == 0:
            send_failure_email_when_AAFS_refresh_running()
...
check_ads_aafs_travel_agency_bq_to_bq_data_refresh_status = PythonOperator(
    task_id='dag_ads_aafs_travel_agency_bq_to_bq_data_refresh_status_check',
...
start >> check_ads_aafs_travel_agency_bq_to_bq_data_refresh_status >> bq_ctl_table_truncate >> task_check_outer_customer_zip_file >> [task_skip_dag_run, task_DummyForward]
