...
DAG_NAME="aafs_customer_file_to_bq_data_load"
...
def outer_success_email_with_restart(environment, email_type, dag_name, product="ads-aafs", assets_tag="ADS:AAFS-LOAD-GCP"):
    ...
    email_body += """This AdMedia DAG, aafs_customer_file_to_bq_data_load with asset tag ADS:AAFS-LOAD-GCP, 
...
def get_error_count_query_from_sql():
    sql_file = open(AAFS_PARAMS_PATH + '/ads-aafs-test/aafs/L 13 select count wads_aafs_optin_error.sql', 'r')
...
def retrieve_file_id_query_from_sql(file_name):
    sql_file = open(AAFS_PARAMS_PATH + '/ads-aafs-test/aafs/L 15 max file from optin_header.sql', 'r')
    query = sql_file.read() \
...
def get_error_txt_from_sql():
    sql_file = open(AAFS_PARAMS_PATH + '/ads-aafs-test/aafs/R 17 select wads_aafs_optin_error.sql', 'r')
    query = sql_file.read().replace("{{ params.PIPELINE_PROJECT_ID }}", PIPELINE_PROJECT_ID)
    sql_file.close()
    # print(query)
    return query
...
def check_optin_error_count_for_too_many_vendors_query_from_sql():
    sql_file = open(AAFS_PARAMS_PATH + '/ads-aafs-test/aafs/R 16 count wads_aafs_optin_error.sql', 'r')
    query = sql_file.read().replace("{{ params.PIPELINE_PROJECT_ID }}", PIPELINE_PROJECT_ID)
    sql_file.close()
    # print(query)
    return query
...
def get_optin_header_updates_from_sql():
    sql_file = open(AAFS_PARAMS_PATH + '/ads-aafs-test/aafs/E1 select count optin header.sql', 'r')
