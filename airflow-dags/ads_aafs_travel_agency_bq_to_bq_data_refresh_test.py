...
DAG_NAME = "ads_aafs_travel_agency_bq_to_bq_data_refresh_test"
target_dag_id = "ads_aafs_customer_file_to_bq_data_load_test"
...
my_config_values = read_parameters_from_file(AAFS_PARAMS_PATH + '/ads-aafs-test/aafs/config_aafs_test.yaml',
                                             ENVIRONMENT)
...
dag = DAG(
    ...
    schedule_interval=None,  # SCHEDULE_INTERVAL  #None  # "30 12 * * *" (in UTC time) =  7:30 AM CST Daily 
    max_active_runs=1,
...
def send_failure_email_when_AAFS_load_running():
    ...
    email_type = 'Failure'
    email_subject = """%s %s for ads-aafs ADS:AAFS-REFRESH-GCP %s""" %(ENVIRONMENT, email_type, DAG_NAME)
    
    email_body = """<p style='color:#FF0000'>*** This email has been generated automatically - DO NOT reply directly ***</p>"""
    email_body += """<strong>Failure</strong>:</br>"""
    email_body += """AAFS Refresh DAG cannot run until AAFS Load DAG has completed. This is because the two jobs both populate three of the same target tables and six of the same staging tables.</br></br>"""
    email_body += """<strong>Actions</strong>:</br>"""
    email_body += """1.	Check why AAFS Load DAG (%s) has been running for at least <i>%s</i> hours.  </br>"""%(target_dag_id,hours)
    email_body += """2. Once AAFS Load DAG is stopped (either successfully or in a failed state), <strong>manually trigger</strong> AAFS Refresh DAG to start <strong>from the beginning</strong>."""
...
def dag_run_status_check():
    ...
    while dag_runs[-1].get_state() == 'running' and counter>0: 
        ...
        counter=counter-1
        if counter == 0:
            send_failure_email_when_AAFS_load_running()
...
check_ads_aafs_customer_file_to_bq_data_load_status = PythonOperator(
    task_id='dag_ads_aafs_customer_file_to_bq_data_load_run_status_check',
...
start >> check_ads_aafs_customer_file_to_bq_data_load_status >> delete_data_from_staging_tables >> insert_region_delete_rows_to_stg_refresh
